...
from rs_jepa.diagnostics import DiagnosticRow, run_diagnostic_for_seed, summarize_rows
...
def phase_a_honesty_rows() -> dict[int, DiagnosticRow]:
    cfg = load_config("configs/phaseA.yaml")
    return {seed: run_diagnostic_for_seed(cfg, seed) for seed in range(5)}


def test_synthetic_simulator_is_deterministic_and_heterogeneous() -> None:
...
def test_synthetic_outputs_have_known_criticality_and_levels() -> None:
...
def test_criticality_label_metadata_is_unitless() -> None:
    data = SyntheticHospitalSimulator(small_cfg()).generate()
    assert not (set(data.criticality_inputs) & ABSOLUTE_FORBIDDEN_LABEL_INPUTS)
    assert_criticality_is_unitless(data)


def test_criticality_label_rejects_absolute_threshold_inputs() -> None:
...
def test_phase_a_features_do_not_include_care_load_label_as_feature() -> None:
    data = SyntheticHospitalSimulator(small_cfg()).generate()
...
def test_phase_a_honesty_gates_hold_per_seed(
    seed: int, phase_a_honesty_rows: dict[int, DiagnosticRow]
) -> None:
...
def test_phase_a_honesty_gates_hold_across_five_seeds(
    phase_a_honesty_rows: dict[int, DiagnosticRow],
) -> None:
