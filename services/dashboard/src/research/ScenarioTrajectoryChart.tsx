  ...
  return (
    <div className="space-y-3" aria-label={ariaLabel}>
      <div className="h-[260px] w-full" aria-hidden="true">
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={points} margin={{ top: 18, right: 18, bottom: 4, left: -12 }}>
            ...
            <ReferenceArea x1="16h" x2="20h" fill="#f36b5f" fillOpacity={0.08} ifOverflow="extendDomain" />
      ...
      <div className="flex flex-wrap items-center gap-x-5 gap-y-2 text-caption">
        ...
        <span className="text-status-critical">Fenêtre critique · 16h–20h</span>
      </div>
      <p className="sr-only">Le pic du plan actuel est à 18h. La zone rouge indique la fenêtre critique de 16h à 20h. Le seuil du scénario est fixé à 1 600 SIIPS pour cette recherche.</p>
    </div>
  );
}
...
function ScenarioTooltip({ active, payload, label }: { active?: boolean; payload?: Array<{ name?: string; value?: number; color?: string }>; label?: string }) {
  ...
  return (
    <div className="chart-tooltip">
      ...
      {payload.filter((entry) => ["Plan actuel", "HospitalOS", "Votre option"].includes(entry.name ?? "")).map((entry) => <p key={entry.name} style={{ color: entry.color }}>{entry.name} · {entry.value} SIIPS</p>)}
    </div>
  );
}
