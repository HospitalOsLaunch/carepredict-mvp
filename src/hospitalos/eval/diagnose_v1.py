...
def main(argv: list[str] | None = None) -> int:
    ...
    report = {
        ...
        "weekly_lag_action_variant": {
            "description": (
                "Future action_sequence uses canonical action vectors observed at the same "
                "hours one week earlier; history actions remain internal to WorldModelService."
            ),
...
def action_feed_evidence() -> dict[str, Any]:
    ...
    return {
        "diagnosis": (
            "Historical actions are zeros inside WorldModelService; future open-loop actions "
            "are the action_sequence argument. The baseline evaluator passes zero_actions(), "
            "so hours > origin t use all-zero action vectors."
        ),
